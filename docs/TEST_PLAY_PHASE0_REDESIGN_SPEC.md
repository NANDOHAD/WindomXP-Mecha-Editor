...
| `ChangeWeapon` | `H` | `E` | `@int[152]`と銃／剣状態を同期。 |
...
| 130～155 格闘 | `H` | `P` | 方向選択、誘導130、SwordCancel、復帰は実装。type 57判定は未確定。 |
...
| BURNERSET | ID、frameName、scale、direction、Transform接続 | 第4引数の原作用途、出力値から表示への式 | Phase 4 |
| 武器発射口 | 未保持 | `WEAPONPOINT`、Transform、タイプ別参照 | Phase 3 |
| 攻撃腕 | 未保持 | `ATTACKARMSET`、type 57のボーンと形状 | Phase 3 |
| 武器モデル／ファイル | 未保持 | GUNFILENAME、SWORDFILENAME、弾モデル参照 | Phase 3/4 |
...
| `U-005` | `RunProc*` / `WeaponAttack*`全タイプの引数 | 代表タイプだけ簡易表示 | 命令ID別ハンドラー、生成オブジェクト構造、複数機体実データ | 3/4 |
| `U-006` | type 57のボーン、形状、持続tick、多段、対象 | 距離ベース単発 | `FUN_004d9dd0`以降の判定生成・更新・破棄と実ANI比較 | 3 |
| `U-007` | AttackFlag全ビット、防御、反射、無敵 | 値保持または未実装 | 被弾ハンドラーと状態offsetの読み書き追跡 | 3 |
| `U-008` | SPTのWEAPONPOINT／ATTACKARMSET／ファイル定義 | 未パース | 原作SPTパーサー分岐、実SPT複数機体、利用側関数 | 3 |
| `U-009` | BURNERSET第4引数とBURNER出力の描画式 | Unity Particle／Cone近似 | 原作SPT格納先と描画更新側の参照追跡 | 4 |
